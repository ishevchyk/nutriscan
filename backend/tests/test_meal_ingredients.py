...
async def test_relink_ingredient_resyncs_snapshot_from_new_product(client, auth_headers):
    ...
    resp = await client.patch(
        f"/meals/{meal['id']}/ingredients/{ingredient_id}",
...
async def test_relink_ignores_manual_fields_sent_alongside_product_id(client, auth_headers):
    ...
    meal = await _create_meal(client, auth_headers, ingredients=[{"name": "Placeholder", "input_amount": 30, "calories": 1}])
    ingredient_id = meal["ingredients"][0]["id"]
...
async def test_unlink_preserves_snapshot(client, auth_headers):
    ...
    meal = await _create_meal(client, auth_headers, ingredients=[{"product_id": product["id"], "input_amount": 150}])
    ingredient_id = meal["ingredients"][0]["id"]
...
async def test_manual_edit_without_product_id_key_leaves_link_untouched(client, auth_headers):
    ...
    meal = await _create_meal(client, auth_headers, ingredients=[{"product_id": product["id"], "input_amount": 100}])
    ingredient_id = meal["ingredients"][0]["id"]
...
async def test_edit_grams_zero_or_negative_422(client, auth_headers):
    meal = await _create_meal(client, auth_headers, ingredients=[{"name": "Whatever", "input_amount": 10}])
    ingredient_id = meal["ingredients"][0]["id"]
...
async def test_edit_reflected_in_meal_nutrition_on_next_read(client, auth_headers):
    ...
    ingredient_id = meal["ingredients"][0]["id"]
    assert meal["nutrition"]["per_meal"]["calories"] == 50
