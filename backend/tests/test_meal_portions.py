...
async def _create_meal(client, headers, name="Test Meal", ingredients=None):
    resp = await client.post("/meals", json={"name": name, "ingredients": ingredients or []}, headers=headers)
    return resp.json()
...
async def test_create_list_update_delete_portion(client, auth_headers):
    ...
    list_resp = await client.get(f"/meals/{meal['id']}/portions", headers=auth_headers)
    assert list_resp.status_code == 200
    assert len(list_resp.json()) == 1

    update_resp = await client.patch(
        f"/meals/{meal['id']}/portions/{portion['id']}",
    ...
    delete_resp = await client.delete(f"/meals/{meal['id']}/portions/{portion['id']}", headers=auth_headers)
    assert delete_resp.status_code == 204
    list_after = await client.get(f"/meals/{meal['id']}/portions", headers=auth_headers)
...
async def test_create_portion_grams_zero_or_negative_422(client, auth_headers):
    ...
    resp = await client.post(
        f"/meals/{meal['id']}/portions", json={"name": "Bad", "grams": -5}, headers=auth_headers
    )
...
async def test_update_portion_grams_zero_422(client, auth_headers):
    ...
    portion = (
        await client.post(
            f"/meals/{meal['id']}/portions", json={"name": "Ok", "grams": 10}, headers=auth_headers
        )
    ).json()
    resp = await client.patch(
        f"/meals/{meal['id']}/portions/{portion['id']}", json={"grams": 0}, headers=auth_headers
    )
    assert resp.status_code == 422
...
async def test_setting_is_default_unsets_previous_default(client, auth_headers):
    ...
    portions = (await client.get(f"/meals/{meal['id']}/portions", headers=auth_headers)).json()
...
async def test_update_is_default_true_unsets_others(client, auth_headers):
    ...
    second = (
        await client.post(
            f"/meals/{meal['id']}/portions", json={"name": "Second", "grams": 20}, headers=auth_headers
        )
    ).json()

    await client.patch(
        f"/meals/{meal['id']}/portions/{second['id']}", json={"is_default": True}, headers=auth_headers
    )
...
async def test_portion_ownership_404(client, auth_headers, second_user_headers):
    ...
    portion = (
        await client.post(
            f"/meals/{meal['id']}/portions", json={"name": "Not Yours", "grams": 10}, headers=second_user_headers
        )
    ).json()

    assert (
        await client.get(f"/meals/{meal['id']}/portions", headers=auth_headers)
    ).status_code == 404
    assert (
        await client.patch(
            f"/meals/{meal['id']}/portions/{portion['id']}", json={"grams": 5}, headers=auth_headers
        )
    ).status_code == 404
    assert (
        await client.delete(f"/meals/{meal['id']}/portions/{portion['id']}", headers=auth_headers)
    ).status_code == 404
...
async def test_delete_portion_is_hard_delete_no_restore(client, auth_headers):
    ...
    portion = (
        await client.post(
            f"/meals/{meal['id']}/portions", json={"name": "Gone", "grams": 10}, headers=auth_headers
        )
    ).json()
    ...
    assert (
        await client.patch(
            f"/meals/{meal['id']}/portions/{portion['id']}", json={"grams": 1}, headers=auth_headers
        )
    ).status_code == 404
    assert (
        await client.delete(f"/meals/{meal['id']}/portions/{portion['id']}", headers=auth_headers)
...
async def test_embedded_portion_nutrition_matches_per_100g_times_grams(client, auth_headers):
    ...
    portion = (
        await client.post(
            f"/meals/{meal['id']}/portions", json={"name": "Half", "grams": 50}, headers=auth_headers
        )
    ).json()

    fetched = (await client.get(f"/meals/{meal['id']}", headers=auth_headers)).json()
